...
export async function checkPageForCandidate(
  ...
  // Find existing candidate for this resource or domain
  let candidate = activeCandidates.find(c => 
    c.primaryDomain === resource.domain ||
  ...
  console.log("[ProjectDetection] Candidate status:", candidate.status, "Score:", candidate.score, "THRESHOLDS.MIN_SCORE:", THRESHOLDS.MIN_SCORE)
  if (candidate.status === 'ready' && !candidate.notificationShown) {
...
function calculateCandidateScore(candidate: ProjectCandidate): number {
  ...
  // 3 visits = 20, 5 visits = 30, 10+ visits = 40
  const visitScore = Math.min(40, (candidate.visitCount / 10) * 40)
  score += visitScore

  // Session consistency (30 points max)
  // 2 sessions = 15, 3 sessions = 22, 5+ sessions = 30
  const sessionScore = Math.min(30, (candidate.sessionIds.length / 5) * 30)
  score += sessionScore

  // Resource specificity (20 points max)
  // Multiple specific resources = higher score
  const resourceScore = Math.min(20, (candidate.specificResources.length / 3) * 20)
  score += resourceScore

  // Time span (10 points max)
  // At least 1 hour = 5, 1 day = 10
 ...
 */
export async function markCandidateNotified(candidateId: string): Promise<void> {
  const candidates = await loadCandidates()
  const candidate = candidates.find(c => c.id === candidateId)
  if (candidate) {
